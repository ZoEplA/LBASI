...
class Lexer(object):
    ...
    def peek(self):  
    # only get next values but not set self.pos that + 1.
        peek_pos = self.pos + 1
    ...
    def _id(self):
    # Handle identifiers and reserved keywords (maybe get alphabet)
        result = ''
        while self.current_char is not None and self.current_char.isalnum():
            result += self.current_char
            self.advance()

        token = RESERVED_KEYWORDS.get(result.upper(), Token(ID, result))  # Lower to Upper
        return token
...
class Parser(object):
    ...
    def declarations(self):
        ...
        """
        declarations = []
# part13 change : make it to satisfy many "var"
        while True:
    ...
    def factor(self):
        ...
        if token.type == PLUS:
            ...
            self.next(INTEGER_CONST)
            return Num(token)
        elif token.type == REAL_CONST:
            self.next(REAL_CONST)
            return Num(token)
...
class SemanticAnalyzer(NodeVisitor):
    ...
    def visit_VarDecl(self, node):
        ...
        # We have all the information we need to crnexte a variable symbol.
